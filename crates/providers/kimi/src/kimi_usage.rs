...
//! `usage` → 7d，`limits[]` → 短周期窗口，`totalQuota` → 月度可用/冻结状态。
...
fn minutes_of(window: &serde_json::Value) -> Option<u64> {
    ...
    let mins = match unit {
        ...
        u if u.contains("WEEK") => duration * 60 * 24 * 7,
        u if u.contains("MONTH") => duration * 60 * 24 * 30,
        _ => duration,
    };
    Some(mins)
}

/// `totalQuota.used` 是月度冻结标志，不是普通用量百分比；转成 0/1 状态额度。
fn monthly_quota_from(
    ...
    let Some(limit) = to_u64(detail.get("limit")) else {
        return Some(Quota {
    ...
    let exhausted = to_u64(detail.get("used")).unwrap_or(0) > 0
        || to_u64(detail.get("remaining")).is_some_and(|remaining| remaining == 0);
    let used = u64::from(exhausted);
    Some(Quota {
        provider: provider.into(),
        account_id: id.clone(),
        window: QuotaWindow::Month,
        ...
        limit: 1,
        reset_at: reset_at(detail),
        status: QuotaStatus::from_percent(used as f64 * 100.0),
        note: note.map(String::from),
    })
}
...
pub fn parse_usages(body: &str, provider: &str, id: &AccountId) -> Vec<Quota> {
    ...
    let membership = v.pointer("/user/membership/level").and_then(|x| x.as_str());
    let mut out = Vec::new();
    ...
    if let Some(total) = v.get("totalQuota") {
        if let Some(q) = monthly_quota_from(total, provider, id, membership) {
            out.push(q);
        }
    }
    out
...
mod tests {
    ...
    #[test]
    fn total_quota_is_monthly_availability_state() {
        let available = r#"{"totalQuota":{"limit":100,"remaining":99}}"#;
        let q = parse_usages(available, "kimi", &AccountId("u1".into()));
        assert_eq!(q.len(), 1);
        assert_eq!(q[0].window, QuotaWindow::Month);
        assert_eq!((q[0].used, q[0].limit), (0, 1));
        assert_eq!(q[0].status, QuotaStatus::Ok);

        let exhausted = r#"{"totalQuota":{"limit":"100","used":"1","remaining":"99"}}"#;
        let q = parse_usages(exhausted, "kimi", &AccountId("u1".into()));
        assert_eq!((q[0].used, q[0].limit), (1, 1));
        assert_eq!(q[0].status, QuotaStatus::Exhausted);
    }

    #[test]
    fn empty_total_quota_keeps_unknown_monthly_row() {
        let q = parse_usages(r#"{"totalQuota":{}}"#, "kimi", &AccountId("u1".into()));
        assert_eq!(q.len(), 1);
        assert_eq!(q[0].window, QuotaWindow::Month);
        assert_eq!((q[0].used, q[0].limit), (0, 0));
        assert_eq!(q[0].status, QuotaStatus::Unknown);
    }

    #[test]
    fn monthly_limit_window_is_recognized() {
        let body = r#"{"limits":[{"window":{"duration":1,"timeUnit":"TIME_UNIT_MONTH"},
            "detail":{"limit":100,"remaining":75}}]}"#;
        let q = parse_usages(body, "kimi", &AccountId("u1".into()));
        assert_eq!(q[0].window, QuotaWindow::Month);
        assert_eq!((q[0].used, q[0].limit), (25, 100));
    }
